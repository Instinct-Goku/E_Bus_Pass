<?php
    ...
    if(isset($_POST['name'])){
        ...
        $query = mysqli_query($connect,"insert into pass(name, email, contact, date, dest, password) values ('$name', '$email', '$contact', '$date', '$dest', '$password')");
		$lid = mysqli_insert_id($connect);
		
		$nod = round((strtotime($date) - time())/(60*60*24))+1;
		$result = mysqli_query($connect,"select price from destination where name = '$dest'");
    }
?>
