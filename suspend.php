<?php
    ...
    if(isset($_POST['id'])){
        $id = $_POST['id'];
        $result = mysqli_query($connect, "select * from pass where id = '$id'");
...
<?php
	$dest = $row['dest'];
	$price = mysqli_fetch_assoc(mysqli_query($connect,"select price from destination where name = '$dest'"))['price'];
	$amt = $price * $nod;
	echo $amt;
	mysqli_query($connect,"UPDATE pass SET paid = paid - '$amt' WHERE id = '$id'");
	mysqli_query($connect,"UPDATE pass SET date = CURDATE() WHERE id = '$id'");
